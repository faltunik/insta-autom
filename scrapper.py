...
class Instabot:
    ...
    @staticmethod
    def valid_cred(username, password):
        client = Client()
    ...
    def scrape_url(self, link, filename, amount=25, last_date=-1):
        ...
        self.get_medias_list(userid, filename, amount, start="", last_date=last_date, link= link)
        return

    def get_medias_list(self, userid, filename, amount=50, start="", last_date=-1, link = ''):
        response = []
        run = True
        iteration = 0
